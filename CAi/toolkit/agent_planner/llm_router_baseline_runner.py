...
DEFAULT_BENCHMARKS = (
    "CAi/toolkit/agent_planner/benchmarks/crossdocked_rxnflow_candidates5_targets30.jsonl",
    "CAi/toolkit/agent_planner/benchmarks/litpcba_vina_prepared_15.jsonl",
    "CAi/toolkit/agent_planner/benchmarks/task_generalization_real_v1.jsonl",
)
...
def run_llm_router_baseline(
    *,
    ...
    responses = _load_response_map(_resolve_path(responses_jsonl, root)) if responses_jsonl else {}
    if response_log_path and response_log_path.exists():
        responses.update(_load_response_map(response_log_path))
    api_config = None
    if router_mode == "api":
...
def _load_project_dotenv(project_root: Path) -> None:
    """Load CAi/.env for API baselines without requiring keys on the CLI."""

    try:
        from dotenv import load_dotenv
    except Exception:  # noqa: BLE001 - dotenv is optional for the runner.
        return
    load_dotenv(project_root / "CAi" / ".env", override=False)
...
def _compact_tool_registry(registry: ChemistryToolRegistry) -> list[dict[str, Any]]:
...
def _load_response_map(path: Path) -> dict[str, str]:
    ...
    for item in _read_jsonl(path):
        ...
        if task_id and response is not None:
            responses[str(task_id)] = str(response)
    return responses
