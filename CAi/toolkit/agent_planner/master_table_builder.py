...
DEFAULT_CROSSDOCKED_RECORD = (
    "CAi/toolkit/agent_planner/benchmarks/records/crossdocked_rxnflow_candidates5_targets15.summary.json"
)
DEFAULT_CROSSDOCKED_TARGETS30_RECORD = (
    "CAi/toolkit/agent_planner/benchmarks/records/crossdocked_rxnflow_candidates5_targets30.summary.json"
)
...
DEFAULT_FAILURE_TAXONOMY_BENCHMARK = "CAi/toolkit/agent_planner/benchmarks/failure_recovery_taxonomy_v2.jsonl"
...
DEFAULT_RECORD_OUTPUT = "CAi/toolkit/agent_planner/benchmarks/records/master_baseline_table.summary.json"
...
DEFAULT_AMBIGUOUS_FAILURE_MODE_BENCHMARK = (
    "CAi/toolkit/agent_planner/benchmarks/failure_recovery_ambiguous_evidence_real_or_injected_v2.jsonl"
)
...
DEFAULT_TOOL_ADMISSION_CARDS = "CAi/toolkit/agent_planner/configs/tool_admission_cards.json"
...
DEFAULT_PDBBIND_PREPARED_PILOT = (
    "CAi/toolkit/agent_planner/benchmarks/pdbbindplus_v2020r1_prepared_pilot_30.jsonl"
)
DEFAULT_PDBBIND_PREPARED_PILOT_FALLBACKS = (
    "CAi/toolkit/agent_planner/benchmarks/pdbbindplus_v2020r1_prepared_pilot_17.jsonl",
    "CAi/toolkit/agent_planner/benchmarks/pdbbindplus_v2020r1_prepared_pilot_15.jsonl",
    "CAi/toolkit/agent_planner/benchmarks/pdbbindplus_v2020r1_prepared_pilot_5.jsonl",
)
...
def _failure_recovery_rows(data: dict[str, Any], record_path: Path, project_root: Path) -> list[dict[str, Any]]:
    ...
    for baseline_result in data.get("baseline_results", []):
        ...
        candidate_metrics = _failure_candidate_metrics(full_copilot_cases if baseline == "full_copilot" else [])
        rows.append(
            _ordered_row(
                {
...
def _failure_notes(data: dict[str, Any], baseline: str | None) -> str:
    ...
    if baseline == "full_copilot":
        return str(
            interpretation.get(
                "primary_effect",
                "Execution-guided repair/fallback is enabled for the full_copilot baseline.",
            )
        )
    if baseline == "rule_based_planner":
...
def _ambiguous_failure_mode_repeated_rows(
    *,
    ...
    for detail in payload.get("detail_rows", []):
        if not isinstance(detail, dict) or detail.get("planner_baseline") != "full_copilot":
            continue
    ...
    *,
    ...
    if full_success and rule_success:
        if full_repair_executed:
            return "Both succeed; full_copilot additionally records a verifier-triggered repair trace."
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build the CAi master baseline table from completed artifacts.")
    ...
    parser.add_argument(
        ...
        help="Optional full_copilot result JSON for ambiguous-evidence failure modes. Use an empty string to disable.",
    )
    parser.add_argument(
