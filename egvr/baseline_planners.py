...
EGVR_AGENT = "egvr_agent"
# Compatibility identifier used by previously released experiment artifacts.
FULL_COPILOT = "full_copilot"
LEGACY_FULL_COPILOT = FULL_COPILOT
...
TOOL_STATUS_ONLY = "tool_status_only"
VERIFIER_TARGETED_RETRY_NO_FALLBACK = "verifier_targeted_retry_no_fallback"
SUPPORTED_BASELINES = (
    ALL_TOOL_AGENT,
    FIXED_PIPELINE,
    RULE_BASED_PLANNER,
    ...
    *,
    ...
    if baseline in {EGVR_AGENT, LEGACY_FULL_COPILOT}:
        ...
        return PlannedWorkflow(
            ...
            notes=[
                *base_workflow.notes,
                "EGVR-Agent enables one verifier-guided repair attempt after execution evidence fails a required check.",
            ],
        )
    ...
    if baseline == TOOL_STATUS_ONLY:
        base_workflow = plan_workflow(parsed_task, registry=registry)
        return PlannedWorkflow(
            task_id=base_workflow.task_id,
            planner_type=TOOL_STATUS_ONLY,
            selected_tools=base_workflow.selected_tools,
            tool_sequence=base_workflow.tool_sequence,
            expected_outputs=base_workflow.expected_outputs,
            notes=[
                *base_workflow.notes,
                "Tool-status-only baseline claims success from nominal tool status without evidence gating.",
            ],
        )
    if baseline == VERIFIER_TARGETED_RETRY_NO_FALLBACK:
        base_workflow = plan_workflow(parsed_task, registry=registry)
        return PlannedWorkflow(
            task_id=base_workflow.task_id,
            ...
            notes=[
                *base_workflow.notes,
                "Verifier-targeted retry is enabled, but fallback-tool actions are disabled.",
            ],
        )
