            ...
            tasks.append(
                {
                    ...
                    "metadata": {
                        ...
                        **({"run_seed": int(run_seed), "rxnflow_seed": int(run_seed)} if run_seed is not None else {}),
                        "real_run_notes": "Requires RxnFlow and evaluator tools through a compatible external service.",
                    },
                }
            )
            if len(tasks) >= limit:
        ...
        tasks.append(
            {
                ...
                "metadata": {
                    ...
                    "pocket_center": center,
                    "box_size": box_size,
                    "real_run_notes": "Requires Vina through a compatible external service.",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate real-path EGVR benchmark tasks.")
