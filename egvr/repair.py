...
class SimpleRepairPlanner:
    ...
    def __init__(self, registry: EvidenceToolRegistry | None = None) -> None:
    ...
    def suggest(
        ...
        if "rxnflow" in failed_tools or ("rxnflow" in planned_workflow.selected_tools and all_generation_failed):
            action = self._fallback_denovo_action(
                parsed_task,
                planned_workflow,
                "rxnflow failed or returned no valid candidates.",
            )
        ...
        if "reinvent4_mol2mol" in failed_tools:
            ...
            fallback_action = self._fallback_denovo_action(
                parsed_task,
                planned_workflow,
                "Mol2Mol failed after seed-based optimization.",
            )
        ...
        if (
            "reinvent4_mol2mol" in planned_workflow.selected_tools
            and all_generation_failed
            and "reinvent4_mol2mol" not in failed_tools
        ):
            reduced = _reduced_num_variants(planned_workflow, "reinvent4_mol2mol")
            retry_action = RepairAction(
                action_type="retry_with_reduced_generation_count",
                tool_name="reinvent4_mol2mol",
                reason="Mol2Mol returned no valid candidates; retry once with a smaller generation count.",
                parameters={"num_variants": reduced},
            )
            fallback_action = self._fallback_denovo_action(
                parsed_task,
                planned_workflow,
                "Mol2Mol retry may require a de novo fallback.",
            )
            actions.extend([retry_action, fallback_action])
            _append_step_if_missing(repaired_steps, self._action_to_step(retry_action))
            _append_step_if_missing(repaired_steps, self._action_to_step(fallback_action))
    ...
    def _fallback_denovo_action(
        ...
        return RepairAction(
            ...
            parameters={
                "fallback": True,
                "num_variants": _fallback_num_variants(parsed_task, planned_workflow),
            },
        )
...
def _fallback_num_variants(parsed_task: ParsedTask, workflow: PlannedWorkflow) -> int:
    requested = parsed_task.constraints.max_candidates
    if isinstance(requested, int) and requested > 0:
        return requested

    count_keys = ("num_variants", "num_samples", "num_analogs", "num_decorations", "max_candidates")
    for step in workflow.tool_sequence:
